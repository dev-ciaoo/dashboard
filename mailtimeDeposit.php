...
function TDMailer($bank, $branch, $balancee, $interestR, $days, $date, $uponM, $remarks, $email, $email2, $email3, $email4, $email5, $email6, $email7, $maturityDate){
    ...
    // Define the due date based on the maturity date and the condition for sending the email (7 days before maturity)
    $dueDate = date('Y-m-d', strtotime($maturityDate . ' -7 days'));

    // Check if the current date is exactly 7 days before the maturity date
    if($currentDate == $dueDate){
        ...
        $mail->Subject = "[ INCOMING 1 WEEK ]  MATURITY - $bank ($balancee)";
        ...
        echo "DATA ERROR: The current date is not 7 days before the maturity date.";
    }
}
