...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $query = "INSERT INTO pay_otherdeductions (`employeeId`, `name`, `branch`, `position`, `amount`, `date`)
        VALUES (?,?,?,?,?,?)";
        ...
        mysqli_stmt_bind_param($stmt, "ssssss", $id, $name, $branch, $position, $amount, $date);

        mysqli_stmt_execute($stmt);

        if (mysqli_stmt_affected_rows($stmt) > 0) {
            // Redirect to the page after successful update or insert
            header("Location: payreadotherdeduction.php");
            exit();
        } else {
