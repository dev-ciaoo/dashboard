...
$id = $_GET['id'];
$SELECT = "SELECT * FROM timedeposit WHERE id='$id'";
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-3 row" id="otherBankInputDiv" style="display: none;">
            <label for="otherBank" class="col-md-4 form-label"><strong>Other Bank</strong></label>
            <div class="col-md-5">
              <input type="text" class="form-control" id="otherBank" name="otherBank">
            </div>
          </div>
        <div class="mb-3 row">
      ...
      <div class="modal-body">
        ...
        <div class="mb-3 row">
          <label for="rbalance" class="col-md-4 form-label"><strong>Balance</strong></label>
            <div class="col-md-5">
              <input type="number" class="form-control" id="rbalance" name="rbalance" Required>
            </div>
        </div>
        <div class="mb-3 row">
...
<script>
  function checkOther(selectElement) {
    var otherBankInputDiv = document.getElementById("otherBankInputDiv");
    var otherBankInput = document.getElementById("otherBank");
    var selectElement = document.getElementById('bankName');

    if (selectElement.value === "Other") {
      otherBankInputDiv.style.display = "block";
      otherBankInput.required = true; // Make the input required
    } else {
      otherBankInputDiv.style.display = "none";
      otherBankInput.value = ""; // Clear the input field
      otherBankInput.required = false; // Make the input not required
    }
  }

  document.getElementById('timedepositForm').addEventListener('submit', function(event) {
    event.preventDefault(); // Prevent form submission
    
    var selectElement = document.getElementById('bankName');
    var otherBankInput = document.getElementById('otherBank');

    if (selectElement.value === 'Other' && otherBankInput.value.trim() !== '') {
      var newBankName = otherBankInput.value.trim();
      
      // Check if the option already exists
      var optionExists = Array.from(selectElement.options).some(function(option) {
        return option.value === newBankName;
      });
      
      if (!optionExists) {
        var newOption = new Option(newBankName, newBankName);
...
<script>
    ...
    // Perform calculation
    let uponMaturity = ((balance * interestRate * terms) / 360) * (0.80);
...
<script>
    ...
    // Perform calculation
    let uponMaturity2 = ((balance2 * interestRate2 * (terms2)) / 360) * (0.80);
