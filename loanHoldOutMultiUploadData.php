...
if(isset($_POST['endPrompt']) != ''){
    ...
    //     archiveFile('riskRating', 'riskRating', $holdOutId, 'a_riskRating', $dateToday, $endPrompt, $con);
    // }
...
// BORROWER
$endorsementFile = upload_file($_FILES['endorsement'], 'multiholdout', $holdOutId);
$loanAppFormFile = upload_file($_FILES['loanAppForm'], 'multiholdout', $holdOutId);
$businessPermitFile = upload_file($_FILES['businessPermit'], 'multiholdout', $holdOutId);
$borrowerIdFile = upload_file($_FILES['borrowerId'], 'multiholdout', $holdOutId);
// $bankCertFile = upload_file($_FILES['bankCert'], 'multiholdout', $holdOutId);
$payslipFile = upload_file($_FILES['payslip'], 'multiholdout', $holdOutId);
$brgyClearanceFile = upload_file($_FILES['brgyClearance'], 'multiholdout', $holdOutId);
$proofBillingFile = upload_file($_FILES['proofBilling'], 'multiholdout',$holdOutId);
$bankDepositFile = upload_file($_FILES['bankDeposit'], 'multiholdout',$holdOutId);

// CO-BORROWER
$coBorrowerStatementFile = upload_file($_FILES['coBorrowerStatement'], 'multiholdout',$holdOutId);
$coBorrowerIdFile = upload_file($_FILES['coBorrowerId'], 'multiholdout',$holdOutId);
$coBorrowerProofIncomeFile = upload_file($_FILES['coBorrowerProofIncome'], 'multiholdout',$holdOutId);

// CO-MAKER
$coMakerStatementFile = upload_file($_FILES['coMakerStatement'], 'multiholdout',$holdOutId);
$coMakerIdFile = upload_file($_FILES['coMakerId'], 'multiholdout',$holdOutId);
$coMakerBusinessPermitFile = upload_file($_FILES['coMakerBusinessPermit'], 'multiholdout', $holdOutId);
$coMakerPayslipFile = upload_file($_FILES['coMakerPayslip'], 'multiholdout', $holdOutId);

// DOCUMENTS
$cashflowAnalysisFile = upload_file($_FILES['cashflowAnalysis'], 'multiholdout', $holdOutId);
$promissoryNoteFile = upload_file($_FILES['promissoryNote'], 'multiholdout',$holdOutId);
$disclosureStatementFile = upload_file($_FILES['disclosureStatement'], 'multiholdout',$holdOutId);
$utilizationFile = upload_file($_FILES['utilization'], 'multiholdout', $holdOutId);
$amortizationSchedFile = upload_file($_FILES['amortizationSched'], 'multiholdout',$holdOutId);
$insuranceFile = upload_file($_FILES['insurance'], 'multiholdout', $holdOutId);
// $riskRatingFile = upload_file($_FILES['riskRating'], 'multiholdout', $holdOutId);

// OTHER
$bankCertFile = upload_file($_FILES['bankCert'], 'multiholdout', $holdOutId);
$waiverConfiFile = upload_file($_FILES['waiverConfi'], 'multiholdout', $holdOutId);
$waiverSecrecyFile = upload_file($_FILES['waiverSecrecy'], 'multiholdout', $holdOutId);
$cicFile = upload_file($_FILES['cic'], 'multiholdout', $holdOutId);
$nfisFile = upload_file($_FILES['nfis'], 'multiholdout', $holdOutId);
$otherSupportFile = upload_file($_FILES['otherSupport'], 'multiholdout', $holdOutId);
...
// $riskRatingPath = $riskRatingFile['path'];
...
//   addColumnUpdate($sqlUpdate, "riskRating", $riskRatingPath);
...
// addStatus($sqlUpdate, "riskRatingStatus", $riskRatingSelect, $riskRatingDesc);
...
$dataUpdate = mysqli_insert_id(mysql: $con);
  ...
  if ($updateQuery) {
        ...
        // sendMail($data['riskRating'], $riskRatingPath, $fullname, $riskRatingName);
    ...
    // addToLocalFiles($localFiles, $riskRatingPath);
    // OTHER
