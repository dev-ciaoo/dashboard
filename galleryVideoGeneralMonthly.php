...
<!-- <button id="uploadBtn">Upload</button> -->
    ...
    <h1><b></b>General Monthly Workshop</b></h1>
    ...
    <script>
       const photosPerPage = 3;
        ...
        async function fetchVideoFiles() {
            try {
                const response = await fetch('./galleryGetVideosGeneralMonthly.php');
...
<script>
   document.getElementById('videoUpload').addEventListener('change', () => {
    ...
    fetch('galleryFileUploadGeneralMonthly.php', {
        method: 'POST',
        body: formData
    })
    ...
    .then(data => {
        if (data.success) {
            ...
            data.uploadedFiles.forEach(file => {
                photos.push('./workshop/video/general-monthly/' + file); // Add uploaded videos to the photos array
            });
