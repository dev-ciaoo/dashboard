...
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
    ...
    <script>
        ...
        async function fetchVideoFiles() {
            try {
                const response = await fetch('./galleryGetVideosROPA.php'); // Fetch file list from PHP
                ...
                videoFiles.forEach(file => {
                    photos.push(`./workshop/video/ropa/${file}`);
                });
        ...
        if (photo.endsWith('.mp4') || photo.endsWith('.mov') || photo.endsWith('.MP4') || photo.endsWith('.MOV')) {
            anchor.addEventListener('click', function(event) {
                ...
                closeButton.addEventListener('click', () => {
                   window.location.href="galleryVideoROPA.php";
                });
...
<script>
   document.getElementById('videoUpload').addEventListener('change', () => {
    ...
    fetch('galleryFileUploadROPA.php', {
        method: 'POST',
        body: formData
    })
    ...
    .then(data => {
        if (data.success) {
            ...
            data.uploadedFiles.forEach(file => {
                photos.push('./workshop/video/ropa/' + file); // Add uploaded videos to the photos array
            });
