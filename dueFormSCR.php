...
<html lang="en">
   ...
   <head>
      ...
      <meta name="description" content="SCR Data">
  ...
  function setFileVisibility($file, $inputId, $checkImageId,$buttonOpen,$selectValue,$date) {
      ...
      <?php
         $query1 = "SELECT a.*, m.* FROM scr AS m
                                          LEFT JOIN scrarchive AS a ON m.consLoanId = a.a_consLoanId
            ...
            <div class="col-12 ">
               <div class="bg-white rounded p-2">
                  <ul class="nav nav-tabs nav-fill justify-content-center bg-light" style="border:solid 1px silver">
                    <li class="nav-item ">
                        <a class="nav-link text-dark disabled" data-bs-toggle="tab" id="tab1" href="#scr">SCR</a>
                     </li>
                     <li class="nav-item ">
...
function initializeDataTable(tableId, ajaxUrl, consId) {
   ...
   $(document).on('click', '#consfLetterShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrfLetter.php', '<?php echo $id; ?>');
   });
   $(document).on('click', '#consfLetter2ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrfLetter2.php', '<?php echo $id; ?>');
   });
   $(document).on('click', '#consfLetter3ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrfLetter3.php', '<?php echo $id; ?>');
   });
   // Second Consolidated Demand
   $(document).on('click', '#conssLetterShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrsLetter.php', '<?php echo $id; ?>');
   });
   $(document).on('click', '#conssLetter2ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrsLetter2.php', '<?php echo $id; ?>');
   });
   $(document).on('click', '#conssLetter3ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrsLetter3.php', '<?php echo $id; ?>');
   });
   // Third Consolidated Demand
   $(document).on('click', '#constLetterShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrtLetter.php', '<?php echo $id; ?>');
   });
   $(document).on('click', '#constLetter2ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrtLetter2.php', '<?php echo $id; ?>');
   });
   $(document).on('click', '#constLetter3ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrtLetter3.php', '<?php echo $id; ?>');
   });
   // Final Consolidated Demand
   $(document).on('click', '#consfdLetterShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrfdLetter.php', '<?php echo $id; ?>');
   });
   $(document).on('click', '#consfdLetter2ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrfdLetter2.php', '<?php echo $id; ?>');
   });
   $(document).on('click', '#consfdLetter3ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrfdLetter3.php', '<?php echo $id; ?>');
   });
   // Other Attachment
   $(document).on('click', '#clientReq1ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_clientReq1.php', '<?php echo $id; ?>');
   });
   $(document).on('click', '#clientReq2ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_clientReq2.php', '<?php echo $id; ?>');
   });
   $(document).on('click', '#clientReq3ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_clientReq3.php', '<?php echo $id; ?>');
   });
   // foreclosure
   $(document).on('click', '#consffClosureShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrffClosure.php', '<?php echo $id; ?>');
   });
   // pastdue LITIGATION
   $(document).on('click', '#conspastLitigationShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrpastLitigation.php', '<?php echo $id; ?>');
   });
   // pastdue LITIGATION2
   $(document).on('click', '#conspastLitigation2ShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrpastLitigation2.php', '<?php echo $id; ?>');
   });
   // trasnfer to ROPA
   $(document).on('click', '#consttLitigationShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrttLitigation.php', '<?php echo $id; ?>');
   });
   // prep Consolidated
   $(document).on('click', '#consPrepConsoShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrPrepConso.php', '<?php echo $id; ?>');
   });
   // due and DEMANDABLE
   $(document).on('click', '#consaDemandShowOld', function(){
      initializeDataTable('#oldFile', 'fetch_asa_scrDemand.php', '<?php echo $id; ?>');
   });
</script>
