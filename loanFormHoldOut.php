   ...
   <body oncontextmenu="return false;">
      ...
      <?php
         ...
         <?php
            ...
            $loanAppForm = $rows['loanAppForm'];
            $bankDeposit = $rows['bankDeposit'];
            $borrowerId = $rows['borrowerId'];
            $businessPermit = $rows['businessPermit'];
            $brgyClearance = $rows['brgyClearance'];
            $payslip = $rows['payslip'];
            $proofBilling = $rows['proofBilling'];
            // OTHERS
            $bankCert = $rows['bankCert'];
            $waiverConfi = $rows['waiverConfi'];
            $waiverSecrecy = $rows['waiverSecrecy'];
            // CO-BORROWER
            $coBorrowerStatement = $rows['coBorrowerStatement'];
            $coBorrowerId = $rows['coBorrowerId'];
            $coBorrowerProofIncome = $rows['coBorrowerProofIncome'];
            // CO-MAKER
            $coMakerStatement = $rows['coMakerStatement'];
            $coMakerId = $rows['coMakerId'];
            $coMakerBusinessPermit = $rows['coMakerBusinessPermit'];
            $coMakerPayslip = $rows['coMakerPayslip'];
            // DOCUMENTS
            $promissoryNote = $rows['promissoryNote'];
            $disclosureStatement = $rows['disclosureStatement'];
            $utilization = $rows['utilization'];
            $amortizationSched = $rows['amortizationSched'];
            $insurance = $rows['insurance'];
            $cashflowAnalysis = $rows['cashflowAnalysis'];
            // BORROWER STATUS
            $loanAppFormSelect = $rows['loanAppFormStatus'];
            $businessPermitSelect = $rows['businessPermitStatus'];
            $borrowerIdSelect = $rows['borrowerIdStatus'];
            $payslipSelect = $rows['payslipStatus'];
            $brgyClearanceSelect = $rows['brgyClearanceStatus'];
            $proofBillingSelect = $rows['proofBillingStatus'];
            $bankCertSelect = $rows['bankCertStatus'];
            $bankDepositSelect = $rows['bankDepositStatus'];
            $waiverConfiSelect = $rows['waiverConfiStatus'];
            $waiverSecrecySelect = $rows['waiverSecrecyStatus'];
            // CO BORROWER STATUS
            $coBorrowerStatementSelect = $rows['coBorrowerStatementStatus'];
            $coBorrowerIdSelect = $rows['coBorrowerIdStatus'];
            $coBorrowerProofIncomeSelect = $rows['coBorrowerProofIncomeStatus'];
            // CO MAKER
            $coMakerStatementSelect = $rows['coMakerStatementStatus'];
            $coMakerIdSelect = $rows['coMakerIdStatus'];
            $coMakerBusinessPermitSelect = $rows['coMakerBusinessPermitStatus'];
            $coMakerPayslipSelect = $rows['coMakerPayslipStatus'];
            // DOCUEMENTS STATUS
            $endorsementSelect = $rows['endorsementStatus'];
            $promissoryNoteSelect = $rows['promissoryNoteStatus'];
            $disclosureStatementSelect = $rows['disclosureStatementStatus'];
            $utilizationSelect = $rows['utilizationStatus'];
            $amortizationSchedSelect = $rows['amortizationSchedStatus'];
            $insuranceSelect = $rows['insuranceStatus'];
            $cashflowAnalysisSelect = $rows['cashflowAnalysisStatus'];
            // // OTHER Checkbox
            // $oathTakingCheck = $row['oathTakingCheck'];
            $bankCertCheck = $rows['bankCertCheck'];
            $waiverSecrecyCheck = $rows['waiverSecrecyCheck'];
            $waiverConfiCheck = $rows['waiverConfiCheck'];
            // // Checkbox Status
            ...
            $otherSupport = $rows['otherSupport'];
            $otherSupportSelect = $rows['otherSupportStatus'];
            $otherSupportCheck = $rows['otherSupportCheck'];
            $edit1 = $rows['edit1'];
            $cic = $rows['cic'];
            $nfis = $rows['nfis'];
            ...
            <div class="col-12 ">
               <div class="bg-white rounded p-2">
                  <ul class="nav nav-tabs nav-fill justify-content-center bg-light" style="border:solid 1px silver">
                     ...
                     <li class="nav-item">
                        <a class="nav-link text-dark active" data-bs-toggle="tab" id="tab2" href="#holdout">Hold-Out</a>
                     </li>
                     <li class="nav-item">
                  ...
                  <div class="row">
                     <div class="col-12">
                        <div class="tab-content p-6">
                           ...
                           <form id="salary-form" action="loanHoldOutUploadData.php" method="POST" enctype="multipart/form-data">
...
<script>   
   function handleSearch() {
      ...
      if(bankposition !== "LOAN Assistant" && position !== "BM" && username !== "jabportillo" && username !== "ejcemata" && username !== "dgayac" && username !== "dmsantos" 
         && username !=="scpayac" && department !=="1" && username !== "hmmendoza" && username !== "tjqpasicolan" && username !== "cgluda" && username !== 'rdalvarez'){
