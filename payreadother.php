<?php
    ...
    $sql="SELECT * FROM pay_otherpayment WHERE datedeleted = ''";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while($row = mysqli_fetch_assoc($result)) {
            ...
            <a class='delete-link' href='paydeleteother.php?id=". $row['id'] . "'><i class=' fa-lg fa-solid fa-trash'></i></a>
            </td>"; 
            $tbody .= "</tr>";
        }
    } else {
...
<!-- MODAL -->
    ...
    <div class="container">
        <div class="d-flex justify-content-center align-items-center"><h3>Other Payment</h3></div>
            <div class="d-flex flex-row gap-2 m-2">
                <div class="ms-auto"> <!-- Utilizing ms-auto (margin-left: auto) to push the button to the right -->
                    <a class="otherpayment-link btn btn-primary">Add Other Payment</a>
                </div>
            </div>
...
$('.otherpayment-link').click(function(){
    ...
    $.ajax({
        url: "payotherpaymentform.php?id=" + employeeId,
        type: "post",
        data: {},
...
$('.update').click(function(){
    var id = $(this).data('id');
    $.ajax({
        url: "payupdateotherform.php?id=" + id,
        type: "post",
        data: {},
        success: function(data) {
            $(".modal-title").html('<div class="">Add Other Payment</div>');
