...
function archiveFile($fileKey, $dbField, $loanID, $archiveField, $dateToday, $endPrompt, $con) {
    if (isset($_FILES[$fileKey]) && $_FILES[$fileKey]['error'] == UPLOAD_ERR_OK) {
        ...
        $selectQuery = "SELECT `$dbField` FROM `consolidated` WHERE `consLoanId` = '$loanID'";
        ...
        if ($row = mysqli_fetch_array($selectResult)) {
            ...
            if($endPrompt != ''){
                $insertQuery = "INSERT INTO `consoarchive` (`a_consLoanId`, `$archiveField`, `a_consdateUpload`, `ac_remarks`)
                                                    VALUES 
...
// BOR_POSTER
$loanAppFormFile = upload_file($_FILES['loanAppForm'], 'consolidated',$consLoanId);
$memoAgreementSFile = upload_file($_FILES['memoAgreementS'], 'consolidated',$consLoanId);
$certofEmploymentFile = upload_file($_FILES['certofEmployment'], 'consolidated',$consLoanId);
$latestPayslipFile = upload_file($_FILES['latestPayslip'], 'consolidated',$consLoanId);
$tinFile = upload_file($_FILES['tin'], 'consolidated',$consLoanId);
$clearanceLoanFile = upload_file($_FILES['clearanceLoan'], 'consolidated',$consLoanId);
// CO MAKER 1
$coMaker1File = upload_file($_FILES['coMaker1'], 'consolidated',$consLoanId);
$validSignaturesFile = upload_file($_FILES['validSignatures'], 'consolidated',$consLoanId);
$monthsPayslipFile = upload_file($_FILES['monthsPayslip'], 'consolidated',$consLoanId);
// CO MAKER 2
$coMaker2File = upload_file($_FILES['coMaker2'], 'consolidated',$consLoanId);
$validSignatures2File = upload_file($_FILES['validSignatures2'], 'consolidated',$consLoanId);
$monthsPayslip2File = upload_file($_FILES['monthsPayslip2'], 'consolidated', $consLoanId);
// DOCUMENTS
$deductRemitFile = upload_file($_FILES['deductRemit'], 'consolidated', $consLoanId);
$cashflowScoreFile = upload_file($_FILES['cashflowScore'], 'consolidated', $consLoanId);
$loanAppMemoFile = upload_file($_FILES['loanAppMemo'], 'consolidated', $consLoanId);
$promissoryNoteSFile = upload_file($_FILES['promissoryNoteS'], 'consolidated',$consLoanId);
$disclosureStateSFile = upload_file($_FILES['disclosureStateS'], 'consolidated',$consLoanId);
$amortScheduleSFile= upload_file($_FILES['amortScheduleS'], 'consolidated',$consLoanId);
// LETTER
$consfLetterFile = upload_file($_FILES['consfLetter'], 'consolidated', $consLoanId);
$conssLetterFile = upload_file($_FILES['conssLetter'], 'consolidated', $consLoanId);
$constLetterFile = upload_file($_FILES['constLetter'], 'consolidated', $consLoanId);
$consfdLetterFile = upload_file($_FILES['consfdLetter'], 'consolidated', $consLoanId);
// LETTER2
$consfLetter2File= upload_file($_FILES['consfLetter2'], 'consolidated', $consLoanId);
$conssLetter2File= upload_file($_FILES['conssLetter2'], 'consolidated',$consLoanId);
$constLetter2File= upload_file($_FILES['constLetter2'], 'consolidated',$consLoanId);
$consfdLetter2File= upload_file($_FILES['consfdLetter2'], 'consolidated',$consLoanId);
// LETTER3
$consfLetter3File= upload_file($_FILES['consfLetter3'], 'consolidated',$consLoanId);
$conssLetter3File= upload_file($_FILES['conssLetter3'], 'consolidated',$consLoanId);
$constLetter3File= upload_file($_FILES['constLetter3'], 'consolidated',$consLoanId);
$consfdLetter3File= upload_file($_FILES['consfdLetter3'], 'consolidated',$consLoanId);
// OTHER ATTACHMENT
$clientReq1File = upload_file($_FILES['clientReq1'], 'consolidated', $consLoanId);
$clientReq2File = upload_file($_FILES['clientReq2'], 'consolidated', $consLoanId);
$clientReq3File = upload_file($_FILES['clientReq3'], 'consolidated', $consLoanId);
// LEGAL
$consffClosureFile = upload_file($_FILES['consffClosure'], 'consolidated', $consLoanId);
$conspastLitigationFile = upload_file($_FILES['conspastLitigation'], 'consolidated', $consLoanId);
$conspastLitigation2File = upload_file($_FILES['conspastLitigation2'], 'consolidated', $consLoanId);
$consttLitigationFile = upload_file($_FILES['consttLitigation'], 'consolidated', $consLoanId);
$consPrepConsoFile = upload_file($_FILES['consPrepConso'], 'consolidated', $consLoanId);
$consaDemandFile = upload_file($_FILES['consaDemand'], 'consolidated', $consLoanId);
...
$sqlSelect = "SELECT * FROM `consolidated` WHERE `consLoanId` = '$consLoanId'";
...
if ($data) {
  ...
  $sqlUpdate = "UPDATE consolidated SET";
  //  BOR_POSTER
...
$dataUpdate = mysqli_insert_id($con);
  ...
  if ($updateQuery == true) {
    ...
    if($_SESSION['username'] == 'ctborgonia' || $_SESSION['username'] == 'jcvillanueva' || $_SESSION['department'] == '6' || $_SESSION['position'] == 'BM'
        ...
        $sqlInsert = "INSERT INTO consolidated (`consLoanId`, 
        ...
        $sqlInsert = "INSERT INTO consolidated (`consLoanId`, `loanAppForm`, `memoAgreementS`, `certofEmployment`, 
