...
<div class="forVUpload">
    ...
    <h1><b></b>President's Meeting</b></h1>
...
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
    ...
    <script>
        ...
        const photosPerPage = 4; // Number of photos per page
        const photos = []; // Initialize as an empty array

        // Fetch video files from the directory
        async function fetchVideoFiles() {
            try {
                const response = await fetch('./galleryGetVideosPres.php'); // Fetch file list from PHP
                ...
                videoFiles.forEach(file => {
                    photos.push(`./workshop/video/President/${file}`);
                });
        ...
        if (photo.endsWith('.mp4') || photo.endsWith('.mov') || photo.endsWith('.MP4') || photo.endsWith('.MOV')) {
            anchor.addEventListener('click', function(event) {
                ...
                closeButton.addEventListener('click', () => {
                   window.location.href="galleryVideoPres.php";
                });
